...
class controllerUsers {
   async register(req, res) {
      ...
      if (user) {
         ...
         res.cookie("logged", 1, loggedCookieOptions);
         res.cookie("refreshToken", refreshToken, refreshCookieOptions);
         new Created({ message: "Đăng ký thành công", metadata: { token, refreshToken } }).send(
            res
         );
      }
   }
   ...
   async getAdminStats(req, res) {
      try {
         ...
         const userGrowth =
            previousPeriodUsers > 0 ? ((newUsers / previousPeriodUsers) * 100).toFixed(1) : 100;
         ...
         const postGrowth =
            previousPeriodPosts > 0 ? ((newPosts / previousPeriodPosts) * 100).toFixed(1) : 100;
         ...
         const revenueGrowth =
            previousPeriodRevenue.length > 0 && previousPeriodRevenue[0].total > 0
               ? (
                    ((recentRevenue.length > 0 ? recentRevenue[0].total : 0) /
   ...
   async searchKeyword(req, res) {
      ...
      if (!keyword) {
         ...
         return new OK({ message: "Lấy từ khóa tìm kiếm thành công", metadata: hotSearch }).send(
            res
         );
      } else {

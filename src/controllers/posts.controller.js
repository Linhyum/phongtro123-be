...
class controllerPosts {
    ...
    async getPosts(req, res) {
        const { category, priceRange, areaRange, typeNews, page = 1, limit = 5 } = req.query;
        ...
        const totalItems = await modelPost.countDocuments(filter);
    ...
    async searchByAddress(req, res) {
        ...
        // Search posts with location containing the address (case-insensitive)
        const data = await modelPost.find({
            location: { $regex: new RegExp(address, 'i') },
            status: 'active',
        }).sort({ createdAt: -1 });
        ...
        const postsWithUser = await Promise.all(
            data.map(async (item) => {
                ...
                return {
                    ...item._doc,
                    user: user ? { _id: user._id, fullName: user.fullName, avatar: user.avatar } : null
                };
            }),
        );

        return new OK({
            message: 'Search by address successful',
            metadata: postsWithUser,
        }).send(res);
    }
}

...
function routes(app) {
   ...
   /// search by address
   app.get("/api/search-by-address", postRoutes);
   ...
   app.post("/api/upload-images", upload.array("images"), (req, res) => {
      return res.status(200).json({
         ...
         images: req.files.map(
            (file) => `https://phongtro123-be.onrender.com/uploads/images/${file.filename}`,
         ),
      });
   });
   ...
   app.post("/api/upload-image", upload.single("avatar"), (req, res) => {
      ...
      return res.status(200).json({
         ...
         image: `https://phongtro123-be.onrender.com/uploads/images/${file.filename}`,
      });
   });
